...
describe("BadgeToken", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should start with soulbound disabled", async function () {
      ...
      void expect(soulbound).to.be.false;
    });
  });
  ...
  describe("Role Management", function () {
    ...
    it("Should prevent non-admin from granting roles", async function () {
      const MINTER_ROLE = await badgeToken.MINTER_ROLE();
      await expect(badgeToken.connect(user1).grantRole(MINTER_ROLE, user2.address)).to.be.reverted;
    });
  });
  ...
  describe("Minting", function () {
    ...
    it("Should prevent non-minter from minting", async function () {
      await expect(badgeToken.connect(user1).mint(user2.address, BADGE_ID_1)).to.be.reverted;
    });

    it("Should revert when minting to zero address", async function () {
      await expect(badgeToken.connect(minter).mint(ethers.ZeroAddress, BADGE_ID_1)).to.be.revertedWithCustomError(
        badgeToken,
        "InvalidAddress",
      );
    });
  ...
  describe("Soulbound Mode", function () {
    ...
    it("Should allow transfers when soulbound is disabled", async function () {
      await badgeToken.connect(user1).safeTransferFrom(user1.address, user2.address, BADGE_ID_1, 1, "0x");
    ...
    it("Should allow admin to enable soulbound mode", async function () {
      ...
      const soulbound = await badgeToken.soulboundEnabled();
      void expect(soulbound).to.be.true;
    });

    it("Should emit SoulboundModeUpdated event", async function () {
      await expect(badgeToken.setSoulbound(true)).to.emit(badgeToken, "SoulboundModeUpdated").withArgs(true);
    });
    ...
    it("Should prevent non-admin from toggling soulbound", async function () {
      await expect(badgeToken.connect(user1).setSoulbound(true)).to.be.reverted;
    });
  ...
  describe("Metadata Management", function () {
    ...
    it("Should prevent non-admin from setting badge URI", async function () {
      await expect(badgeToken.connect(user1).setBadgeURI(BADGE_ID_1, "ipfs://test")).to.be.reverted;
    });
  ...
  describe("ERC-1155 Compliance", function () {
    it("Should support ERC1155 interface", async function () {
      ...
      void expect(supportsInterface).to.be.true;
    });
    ...
    it("Should support batch transfers when soulbound is disabled", async function () {
      ...
      await badgeToken
        .connect(user1)
        .safeBatchTransferFrom(user1.address, user2.address, [BADGE_ID_1, BADGE_ID_2], [1, 1], "0x");
  ...
  describe("Edge Cases", function () {
    ...
    it("Should support approval mechanisms", async function () {
      ...
      await badgeToken.connect(user2).safeTransferFrom(user1.address, user2.address, BADGE_ID_1, 1, "0x");
